...
st.set_page_config(page_title="InvestAI: Smart Mutual Fund Advisor", layout="wide")

# Title and introduction
st.markdown("## 🎯 Welcome to InvestAI – Your Smart Mutual Fund Advisor")
st.markdown("Use AI to get hyper-personalized mutual fund recommendations in seconds!")
...
with st.expander("🧾 Step 1: Enter Your Investor Profile", expanded=True):
    ...
    with col5:
        goal = st.selectbox("🎯 Investment Goal", ["Wealth Creation", "Retirement", "Education", "Travel"])
    with col6:
...
if st.button("🚀 Get My Investment Plan"):
    ...
    st.markdown("## 📊 Top Mutual Fund Recommendations")
    for _, fund in recommended_funds.iterrows():
        with st.expander(f"📈 {fund['Fund Name']}"):
